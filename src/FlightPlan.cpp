...
bool FlightPlan::complete() const {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling complete");
    return !(fDeparture == -1 or fArrival == -1 or
            fInterval == -1 or fDestination.empty());
}

const string &FlightPlan::getDestination() const {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling getter/setter");
    return fDestination;
}
...
void FlightPlan::setDeparture(int departure) {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling setDeparture");
...
void FlightPlan::setArrival(int arrival) {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling setArrival");
...
void FlightPlan::setInterval(int interval) {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling setInterval");
    REQUIRE(interval > 0, "Interval has to be at least 1");
    FlightPlan::fInterval = interval;
}

EEvent FlightPlan::getEvent(Time time) {
    REQUIRE(this->properlyInitialized(), "FlightPlan wasn't properly initialized when calling getEvent");
    // Start at 12:00
    Time currentTime(12);
