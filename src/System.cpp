...
void System::prepare(Airplane *plane, ostream& fLog) {
    ...
    if (request == kAccepted) {
        ...
        stringstream stream;
        stream << plane->getSquawk();
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, "Cleared to " + dest->getName() + ", initial altitude five thousand, expecting one zero zero in ten, squawking " + stream.str() + ", " + planeCS + "."));
...
void System::pushback(Airplane *plane, ostream &fLog) {
    ...
    if (request == kAccepted) {
        // response
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, "Pushback approved, " + planeCS + "."));
    ...
    // If plane hasn't sent a request yet, or the previous request was denied
        ...
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, ss.str()));
        plane->setTimeRemaining(1);
        plane->setRequest(kPending);
        return;
    }
...
void System::taxiDepartureStart(Airplane *plane, ostream &fLog) {
    ...
    if (request == kAccepted) {
        ...
        // Send new request to ATC, which takes 1 minute, and we change the status of the request
        fATC->sendRequest(fTime, plane);
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, planeCS + " is ready to taxi."));
...
void System::atRunway(Airplane *plane, ostream &fLog) {
    ...
    if (request == kAcceptedImmediate) {
        ...
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, "Runway " + dest->getName() + " cleared for take-off, " + planeCS + "."));
        return;
    }
    ...
    if (request == kAccepted) {
        ...
        plane->setRequest(kIdle);
        plane->setTimeRemaining(1); // line up
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, "Lining up runway " + dest->getName() + " and wait, " + planeCS + "."));
        return;
    }
    ...
    // If plane hasn't sent a request yet, or the previous request was denied and plane has waited
        ...
        // Send new request to ATC, which takes 1 minute, and we change the status of the request
        fATC->sendRequest(fTime, plane);
        fATC->sendMessage(fATC->formatMessage(fTime, planeCS, portCS + ", " + planeCS + ", holding short at " + dest->getName() + "."));
        plane->setTimeRemaining(1);
        plane->setRequest(kPending);
        return;
    }
...
void System::onRunway(Airplane *plane, ostream &fLog) {
...
void System::takeoff(Airplane *plane, ostream& fLog) {
